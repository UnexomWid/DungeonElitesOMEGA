		...
		void ApplyNow (List<GraphNode> nodes) {
			...
			if (nodes != null) {
				...
				for (int i = rndStart; i < nodes.Count; i += rnd.Next(1, randomStep)) {
					...
					/*List<GraphNode> bfsNodes = PathUtilities.BFS(nodes[i], 0);
					Debug.Log(bfsNodes.Count);
					for(int j=0;j<bfsNodes.Count;j++)
                    {
						bfsNodes[j].Penalty = (uint)(bfsNodes[j].Penalty + penalty);
						prevNodes.Add(bfsNodes[j]);
					} Implementarea mea lol */
